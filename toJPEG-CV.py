...
def cambiarName():
    ...
    imagesJPG = np.array([file for file in os.listdir() if file.endswith('.jpeg')])
    cont = 1
...
def main():
    ...
    images_PNG = np.array([file for file in os.listdir() if file.endswith('.png')])

    print("Archivos a modificar! :")
    print(images_PNG)

    qlt = int(input("Ingresa la calidad del 1-100: "))
    params=[cv2.IMWRITE_JPEG_QUALITY, qlt, cv2.IMWRITE_JPEG_PROGRESSIVE, 1, cv2.IMWRITE_JPEG_OPTIMIZE, 1]
    ...
    while len(images_PNG) != 0: #PNG
        ...
        cv2.imwrite(f'{image_name}.jpeg', imageCV, params)
